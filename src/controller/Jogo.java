...
//printa o baralho
	...
	//printa as regi√µes e seus estados
	public void debug() {
		ArrayList<Regiao> AuxReg= Mapa.getRegioes();
		for (int i = 0;i < AuxReg.size(); i++) {
	...
	public void proximoJogador() {
		if (jogadorDoTurno.getId() == Jogadores.size()) {
			...
			jogadorDoTurno = Jogadores.get(jogadorDoTurno.getId()); // passa para o proximo jogador j· que o ID comeÁa em 1 e a posiÁ„o comeÁa em 0
		}		
	...
	public void inicioDeTurno(Jogador jogador, int tropasExtras) throws Exception {
		...
		boolean check = false;	// mantem no laÁo atÈ entrar com um dado v·lido	
		String Escolhas[];
		Object n[];		
//		Checando entradas
		do {
